...
const Stories = ({ stories, getUserHandler }) => {
  ...
  return (
    <React.Fragment>
      <section className="stories" data-testid="stories">
        <div className="container">
          {stories.map((story, index) => {
            const perfil=getUserHandler(story.userId);

            return (
              <button className={`user__thumb ${index===0 && 'user__thumb--hasNew'}`}
                key={story.id} onClick={() => handleStory(story)}>
                <div className="user__thumb__wrapper">
                  <img src={perfil.avatar} alt={perfil.name}></img>
                </div>
              </button>
            )
          })}
      ...
      {showStory && (
        <Story story={selectedStory} user={selectedProfile}
